...
- **System engine** — schema defined here, not in the type registry.
...
python3 users/<id>/tools/scripts/resolve_knowledge.py --topic domain.field
python3 users/<id>/tools/scripts/resolve_knowledge.py --applies-to project.jah
python3 users/<id>/tools/scripts/resolve_knowledge.py --source-id source.foo
```
...
| Modify `system/knowledge_base.md` | L5 — explicit request + PR |
