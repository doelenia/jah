...
try:
    from knowledge_lib import knowledge_base_dir, match_entries
except ImportError:
    knowledge_base_dir = None  # type: ignore
    match_entries = None  # type: ignore
...
def compile_matched_knowledge(
    ...
    parts.append(
        "Resolve full entry: "
        f"`python3 users/{active_principal()}/tools/scripts/resolve_knowledge.py <id>`"
    )
    return "\n".join(parts) + "\n"
...
- Run **scope expansion** for project `{project_name}` (`system/discovery_protocol.md` § Scope expansion).
- Resolve types for every instance you will touch (`system/type_system.md`).
...
def main() -> int:
    ...
    sections = [
        ...
        "## Agent Protocol",
        "",
        load_md_block(root / "system/discovery_protocol.md", "Discovery Protocol"),
        load_md_block(root / "system/knowledge_base.md", "Knowledge Base"),
        load_md_block(root / "system/proactive_capture.md", "Proactive Capture"),
        load_md_block(root / "system/open_structure.md", "Open Structure"),
        "## System Principles",
        "",
        load_md_block(root / "system/glossary.md", "Glossary"),
        load_md_block(root / "system/protocol.md", "Protocol"),
        load_md_block(root / "system/type_system.md", "Type System"),
        "## Scoped Types",
        "",
        ...
        "",
        "**External (L6–L7):** external tools, publish, send — explicit approval only.",
    ]
