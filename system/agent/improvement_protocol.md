...
```bash
python3 system/release/scripts/bump_release.py --bump patch -m "What changed"
```

One system commit = one semver entry in `system/release/CHANGELOG.md`.
...
| Connectors | `users/<id>/connectors/` (optional) | Auto |
