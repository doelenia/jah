...
Container instances declare typed child fields in a `fields:` block — **baseline + grown fields** (`open_structure.md`). The type declares a minimal baseline; the instance is authoritative for shape. Resolve each field's type through `registry.yaml` — see `type_system.md`.
...
Source files remain in scope `sources/files/`. Knowledge entries link via `grounded_in` and `canonical_path`. Protocol: `knowledge_base.md`.
